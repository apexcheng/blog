...
```yaml
name: blog-brief
description: 为个人技术博客生成写作 brief。用户提到写文章、整理经验、改写旧文、做视觉笔记、沉淀 Agent 工作流、RPA 经验或数据处理案例时使用。输出包含目标读者、文字风格、核心结论、素材清单、视觉笔记结构、需要确认的问题和验证方式。
```
...
## 输出格式
固定输出 brief，包含标题候选、文字风格、核心结论、视觉笔记结构、素材、风险、下一步。
```
...
1. 先把常用 prompt 放进 `docs/agent/blog-skills/05-prompt-library.md`。
2. 连续使用几次，观察哪些真的重复。
3. 再把稳定、边界清楚的流程升级成 Skill。

## 6. 写作能力分类法

`awesome-agent-skills` 更像技能目录。可借鉴它的分类方式，但按本博客实际保留项拆成这些小块：

- 写作：brief、outline、write、rewrite。
- 结构：visual-note outline、Mermaid、组件选择、标题和段落。
- 事实：fact-check、强断言处理、不编造来源标题。
- 审稿：sensitive-info-check、placeholder-check、AI 味清理。
- 维护：content index、旧文重写、文章类型归档。
- Skill 化：评估哪些流程值得从 prompt 升级成 Skill。

当前不把社媒分发、重 SEO 评分和复杂 delivery contract 放进默认流程。
